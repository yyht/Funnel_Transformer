...
def input_embedding(net_config, initializer, inputs, is_training, seg_id=None, pos_id=None,
	...
	##### Embedding
	def embed_func(x, pos_id, seg_id, initializer,
						embedding_table_adv, embedding_seq_adv,
						emb_adv_pos, stop_gradient):
	...
	with tf.variable_scope(scope, reuse=reuse):
		...
		word_emb = funnel_transformer_ops.layer_norm_op(
									word_emb, 
									norm_shape=[net_config.d_embed])
					...
					for repeat_idx in range(cur_repeat_size):
						...
						if do_pooling:
							...
							pool_ret = funnel_transformer_utils.post_attn_pooling(
									...
									is_training, post_attn_pooling_name)
